...
/*   Updated: 2024/01/20 21:25:42 by hboudar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	sort_with_algo(t_stack **a, t_stack **b, int size, int len)
{
	...
	{
		((*a)->dest <= size / 2) && (op(a, b, 'p', 'b'));
		((*a)->dest > size / 2) && (op(a, b, 'p', 'b'), op(a, b, 'r', 'b'));
		len++;
	}
...
static int	sort_stack(t_stack **a, t_stack **b, int size)
{
	...
	while (size >= 4 && *a != min)
	{
		(min->pos <= size / 2) && (op(a, b, 'r', 'a'));
		(min->pos > size / 2) && (op(a, b, 'o', 'a'));
	}
	return (1);
}
