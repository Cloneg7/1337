...
/*   Updated: 2023/12/17 22:46:34 by hboudar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	ft_count_nbr(long int nbr, t_menu *menu, int op)
{
	int	i;
...
static void	ft_treat_flag(long int *n, t_menu *menu)
{
	...
	while (menu->lenght-- - ft_count_nbr(*n, menu, 1) > 0)
	{
		menu->count += ft_write_c('0', menu->fd);
		menu->width--;
	}
	ft_count_nbr(*n, menu, 2);
	menu->width -= ft_count_nbr(*n, menu, 1);
}
...
void	ft_precision_di(long int n, t_menu *menu)
{
	...
	{
		if ((n < 0 || menu->plus || menu->space)
			&& menu->lenght > ft_count_nbr(n, menu, 1))
			menu->width--;
